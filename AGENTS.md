...
- Prefer the global player embedded in the header nav (centered among adjacent controls) rather than a footer bar; it can appear and disappear without slide animation. On mobile, stack `HeaderPlayer` into two rows (cover + single-line scrolling artist/title marquee + like/playlist/viz on top; transport + compact waveform instead of a seek bar below); on desktop keep two-line artist/title, with marquee for overflow.
...
- Tenant hosting needs a Route 53 `*.sndbnk.com` A record at the Lightsail IP (deploy smoke-tests it). Custom-domain verify accepts CNAME to `{user}.sndbnk.com` or A/AAAA/ALIAS to platform edge IPs (apex-safe); Settings shows those IPs; `example.com` ↔ `www.example.com` are paired for routing/TLS after one is active.
