...
- Every paged track listing shares one kit: keyset cursor helpers in `src/lib/server/cursor.js` (bidirectional, optionally inclusive), `serializeTrackRows()` in `src/lib/server/tracks.js`, a single `GET /api/tracks` with `feed` / `library` / `profile` scopes returning mixed `{ kind: 'track' | 'playlist' }` items, and on the client `TrackList` (`src/lib/lists/track-list.svelte.js`), the `whenVisible` attachment (`src/lib/lists/infinite-scroll.js`), `InfiniteList.svelte`, and `restorableList()` paired with SvelteKit's `export const snapshot`. Feed supports a `q` search beside the ALL/FOLLOWING scope strip (accent-shadowed input, overlay clear `X` when active—no separate clear chrome). Playlists (`playlist` / `playlist_track` / `playlist_like`) are ordered collections of any published tracks; `PlaylistCard` uses one waveform chrome plus a connected member list; comments post to the active track; playlist likes are separate from track likes; routes under `/playlists/*` and `/api/playlists/*`.
