import { fail } from '@sveltejs/kit';

import { createPlaylistFromForm } from '#lib/server/playlists';
import { getProfileByUserId } from '#lib/server/tenant';
import { safeRedirect } from '#lib/server/safe-redirect';

export const load = async ({ locals }) => {
	if (!locals.user) {
		safeRedirect(302, '/signin');
	}

	const profile = await getProfileByUserId(locals.user.id);
	if (!profile) {
		safeRedirect(302, '/signup');
	}

	return {
		user: {
			id: locals.user.id,
			name: locals.user.name
		},
		profile: {
			username: profile.username
		}
	};
};

export const actions = {
	default: async ({ locals, request }) => {
		if (!locals.user) {
			safeRedirect(302, '/signin');
		}

		const formData = await request.formData();
		const result = await createPlaylistFromForm(locals.user.id, formData);

		if (!result.ok) {
			return fail(400, {
				message: result.message,
				title: formData.get('title')?.toString() ?? '',
				description: formData.get('description')?.toString() ?? '',
				published: formData.get('published')?.toString() !== 'false'
			});
		}

		safeRedirect(303, `/playlists/${result.playlistId}/edit`);
	}
};
