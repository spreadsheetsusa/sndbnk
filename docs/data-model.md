...
| Column                                      | Purpose                                                                                                                                                                                                             |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `maxTracks` / `maxLocalBytes`               | Caps (`null` = unlimited). Free seeds **15 tracks** / no byte cap; paid tiers seed unlimited tracks + hosted GiB. Bytes meter hosted adapters (`local` + platform `s3`); SSH excluded. Track count is all adapters. |
| `allowStorageAdapters`                      | BYO (SSH/SFTP today; creator S3/R2 stubbed `enabled: false`) — true on every seeded tier                                                                                                                            |
